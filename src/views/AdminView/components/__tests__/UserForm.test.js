...
describe('<UserForm />', () => {
  ...
  it('correctly renders wrapper table without any rows', () => {
    ...
    expect(wrapper.type()).toBe(UserForm);
  ...
  it('opens user dialog', () => {
    expect(wrapper.find('.new-user-dialog-content')).toHaveLength(0);
    wrapper.instance().handleUserDialog();
    wrapper.update();
    expect(wrapper.find('.new-user-dialog-content')).toHaveLength(1);
  });
  ...
  describe('calls prop methods', () => {
    ...
    it('editUser', () => {
      ...
      expect(editUser.mock.calls.length).toBe(0);
    ...
    it('deleteUsers', () => {
      ...
      expect(deleteUsers.mock.calls.length).toBe(1);
    });
  });
  ...
  it('manages userGroups state correctly', () => {
    ...
    wrapper.update();
    expect(wrapper.state().newUserGroups.length).toBe(1);
    wrapper.instance().handleNewUserGroup({ '@rid': 'new usergroup' });
    wrapper.update();
    expect(wrapper.state().newUserGroups.length).toBe(0);
  });
